...
const navLink = "text-[var(--txt-soft)] text-[0.95rem] uppercase tracking-[0.06rem] font-['Orbitron',sans-serif] relative transition-colors duration-200 after:content-[''] after:absolute after:left-0 after:-bottom-2 after:w-full after:h-0.5 after:bg-[linear-gradient(90deg,var(--glow-cyan),var(--glow-pink))] after:scale-x-0 after:origin-left after:transition-transform after:duration-200 hover:text-[var(--txt-main)]";
const activeNavLink = `${navLink} text-[var(--txt-main)] after:scale-x-100`;
const navButton = "min-w-[190px] h-[42px] px-[18px] gap-2.5 border border-[rgba(255,62,136,0.65)] rounded-[10px] bg-transparent text-[var(--txt-main)] uppercase cursor-pointer font-['Orbitron',sans-serif] tracking-[0.05rem] flex items-center justify-center no-underline";
const languageOption = "flex items-center gap-3 bg-transparent border-0 text-[var(--txt-main)] py-2.5 px-3.5 text-left cursor-pointer font-['Orbitron',sans-serif] uppercase tracking-[0.05rem] rounded-lg whitespace-nowrap hover:bg-[rgba(255,62,136,0.15)] hover:text-[var(--glow-cyan)]";
...
const flag = "w-7 h-[22px] rounded-md object-cover border border-[rgba(255,62,136,0.45)] shadow-[0_0_10px_rgba(255,62,136,0.25)]";
	...
	return (
		<header className="h-18 flex items-center justify-between border border-(--line-soft) rounded-2xl bg-[rgba(8,14,32,0.7)] backdrop-blur-[6px] px-6 relative z-11 max-[1100px]:h-auto max-[1100px]:p-3.5 max-[1100px]:gap-3 max-[1100px]:flex-wrap">
			<div className="flex items-center gap-2.5 text-[1.2rem] font-bold font-['Orbitron',sans-serif] tracking-[0.05rem]">
				<NavLink to="/" className="text-(--txt-main)">{t("navbar.brand.main")}</NavLink>
				<NavLink to="/" className="text-(--glow-pink) [text-shadow:0_0_10px_rgba(255,62,136,0.6)]">{t("navbar.brand.accent")}</NavLink>
			</div>
