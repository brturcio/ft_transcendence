...
async function main() {
  ...
  // Import after .env loading to avoid eager env validation failures in local scripts.
  const { db, pool } = await import("../src/config/db");
  const { achievements, friendships, messages, userStats, users } = await import("../src/db/schema");
